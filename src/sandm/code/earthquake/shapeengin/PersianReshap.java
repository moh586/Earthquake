package sandm.code.earthquake.shapeengin;

import sandm.code.earthquake.MainActivity;



/**
 * Created by Joneidi on 6/2/13.
 */
public class PersianReshap
{
    ...
    static char[] nums={0x0660,0x0661,0x0662,0x0663,0x0664,0x0665,0x0666,0x0667,0x0668,0x0669};
    ...
    public static String reshape(String Str){
        ...
        for(int i=0;i<Str.length();i++)
        {
            char c=Str.charAt(i);
            if(c>='0'&&c<='9')
                Str=Str.substring(0,i)+String.valueOf(nums[c-48])+Str.substring(i+1,Str.length());
        }
        //
        if(MainActivity.Reshape==0)
        	return Str;
        //
        String Temp=" "+Str+" ";
        char pre,at,post;
